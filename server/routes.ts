...
- Return ONLY the JSON, no explanation`
  ...
  // POST /api/estimates/:id/regenerate-link — invalidate old client link, issue new uniqueId
  app.post("/api/estimates/:id/regenerate-link", requireAuth as any, async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      const existing = await storage.getEstimate(id);
      if (!existing) return res.status(404).json({ error: "Estimate not found" });
      const newUniqueId = generateUniqueId();
      const updated = await storage.updateEstimate(id, { uniqueId: newUniqueId });
      await storage.logActivity({
        estimateId: id,
        userId: (req as any).user?.id,
        action: "link_regenerated",
        details: `Client link regenerated by ${(req as any).user?.name || "user"} — old link disabled`,
      });
      res.json({ ok: true, uniqueId: newUniqueId, estimate: updated });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });
